...
async function updateCont_ana(ctrl, Cont_ana) {
    const conn = await conectar(ctrl);
    const sql = "UPDATE cont_ana SET descricao=?, docs=?, dt_lanc=?, estabelecimento=?, id_cont_ana=?, tipo=?, versao=? WHERE id_cont_ana = ? AND versao = ?";
    const [rows] = await conn.query(sql, [
        Cont_ana.descricao,
        Cont_ana.docs,
        (Cont_ana.dt_lanc === null ||Cont_ana.dt_lanc.length === 0) ? null : Cont_ana.dt_lanc.split("-").reverse().join("-"),
...
async function deleteCont_ana(ctrl, Cont_ana) {
    const conn = await conectar(ctrl);
    const sql = "DELETE FROM cont_ana WHERE id_cont_ana = ? AND versao = ?";
    const [rows] = await conn.query(sql, [
        parseInt(Cont_ana.id_cont_ana),
        parseInt(Cont_ana.versao),
    ]);
